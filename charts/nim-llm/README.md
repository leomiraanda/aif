...
Deploys an NVIDIA NIM large language model inference service. Default model: Llama 3 8B Instruct.

## Requirements

- NVIDIA GPU node with driver and device plugin installed
- Node labeled `nvidia.com/gpu.present=true`
- Default: 1 GPU, 8 CPU, 32Gi memory

## Health Checks

The chart configures liveness and readiness probes against `/v1/health/ready` (port 8000).
NIM containers typically require 60+ seconds to load model weights before becoming ready.
...
| `resources.requests.memory` | string | `32Gi` | Memory request |
| `resources.limits.cpu` | string | `8` | CPU limit |
| `resources.limits.memory` | string | `32Gi` | Memory limit |
| `resources.limits.nvidia.com/gpu` | string | `"1"` | GPU limit |
