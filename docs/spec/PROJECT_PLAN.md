...
> **Follow-up (post-merge, from smoke-e2e harness work 2026-05-28):**
>
> 1. **`make helm-install` is a stub; smoke-e2e bypasses the chart entirely.** The smoke harness (`hack/smoke-e2e.sh`, PR #62) runs the operator binary out-of-process so the chart's cert-manager dependency isn't required on the throwaway k3d cluster. As a result, the chart skeleton this story lands is exercised only via `helm lint` + `helm template` until either (a) the chart's webhook TLS mode is parameterised (P0-7) and the smoke harness opts into `webhook.tlsMode=helm-hook`, or (b) cert-manager is installed during smoke-e2e bring-up. Today the gap is implicit in `docs/dev/validation.md`; promote it to first-class once P0-7 lands and add an AC: "`make helm-install` brings up the chart on the smoke cluster in `webhook.tlsMode=helm-hook` mode; `hack/smoke-e2e.sh --via-chart` opts into that path so a regression in the templated install fails CI rather than slipping past the out-of-cluster code path."
...
> **Follow-up (post-merge, from smoke-e2e harness work 2026-05-28):**
>
> 1. **Call `ctrl.SetLogger` in `internal/manager/setup.go` to silence the controller-runtime warning.** Smoke runs produce noisy `[controller-runtime] log.SetLogger(...) was never called; logs will not be displayed (nil)` warnings before every reconcile. The fix is one line in `NewManager` — wire controller-runtime's logr through the existing `slog` handler (e.g. `ctrl.SetLogger(logr.FromSlogHandler(slog.Default().Handler()))`) before `ctrl.NewManager`. Cosmetic, but the warning trains future contributors to ignore controller-runtime's log channel, which masks real warnings. Add an AC: "`ctrl.SetLogger` is called from `NewManager` so controller-runtime logs route through `slog`; smoke-e2e harness asserts the `log.SetLogger(...) was never called` string is absent from the operator log tail."
...
> **Follow-up (post-merge #58 — MVP scope reduction, blueprint-as-helm-wrapper dropped):** the "wrap a single vendor Helm chart as a Blueprint" concept is out of scope for the MVP. P2-7 (Vendor-Chart Wrapper) is **deferred to post-MVP** and the engine-level `ChartAnnotations` + `App.ReferenceBlueprint` plumbing landed in P2-5 stays in place as scaffolding for the deferred consumer. The cleanup slice (file as one PR, types-first) is: (1) delete `BlueprintSourceType`, `BlueprintSource`, and `WrapsVendorChartRef` from `api/v1alpha1/blueprint_types.go`; drop the `Source` field on `BlueprintSpec`; run `make manifests generate`; (2) drop the corresponding webhook enum/discriminated-union validators; (3) drop the wraps-vendor-chart branch from `internal/controller/blueprint_controller.go` if/once it exists; (4) drop the `aif.pages.blueprints.card.origin.wrapsVendorChart` l10n key and the `Origin: Published from Bundle / Wraps vendor chart` rendering from `ui/ai-factory/.../pages/blueprints.vue`; (5) collapse the `source.type` validation, `blueprint-source` label derivation, and `Published`-defaulting added in PR #58 commit `b9854c3` — a ~10-line revert in `internal/api/blueprints.go`; (6) drop the `TestBlueprintsCreate_RejectsWrapsVendorChartSource` / `_DerivesSourceLabel` / `_DefaultsSourceTypeToPublished` tests. **Approval trail:** boss raised the scope question on 2026-05-28 ("we wanted to wrap/show some helmcharts as blueprints (with single helm chart) — we don't need that anymore at least for the MVP"); the user confirmed the defer-the-CRD-shrink, land-the-defensive-validation sequencing ("Changing validation rules at the API is cheap enough for us to defer the decision. What I want to do asap is to wrap up the 'alternative' plans, even if that means having some transient inconsistencies") and authorized this follow-up note ("Proceed").
...
>
> 3. **No admission webhook validates `(authType, credSecretRef)`.** The CRD enum on `FleetConfig.AuthType` allows `""` (the field is `omitempty`), and `internal/webhook/` has no validator for `SettingsConfig` / `FleetConfig`. A misconfiguration like `authType=token` with no `credSecretRef`, or `credSecretRef` set with `authType=""`, currently passes admission silently. The reconciler's `translateSettings` switch falls through to a zero `FleetGitAuth`, the bus projects an anonymous transport, and `pkg/git` either succeeds against a public repo or fails with a transport error on a private one — never with `Ready=False, Reason=InvalidSpec` on the Settings CR. Add a validating webhook in `internal/webhook/` (matches CLAUDE.md "cross-field invariants live in admission webhook") in whichever later story owns Settings admission validation; the post-merge audit on P5-4b chose to fix only the in-source comment now and defer the webhook here.
